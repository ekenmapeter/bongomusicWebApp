<!-- Trending -->
        <div class="grid md:grid-cols-2 grid-cols-1 gap-2 w-full space-y-4 p-2">
            @forelse($just_added as $data)
           
            <div href="#" class="lg:flex xl:flex md:flex flex items-center bg-white border rounded-lg shadow-md md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
                <div class="w-24">
                <img
                ...
                <div class="w-48 grid-cols-1">
                    <h5 class="mb-0 text-sm font-bold tracking-tight text-gray-900 dark:text-white">{{ Str::limit($data->title , 19) }}</h5>
                    <p class="mb-2 text-xs font-semibold text-gray-700 dark:text-gray-400">{{ Str::limit($data->artist_name , 15) }} </p>
                </div>
