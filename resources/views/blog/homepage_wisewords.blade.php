<div class="bg-white shadow-lg border p-5">
          ...
          C60.338,188.266,55.714,189.346,53.527,192.864z"
                        ...
                        <Link href="#" class="font-semibold inline-block">Wise Words</Link>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                    <div class="w-full border-b">
                        <Link href="{{ route('blog.single', ['slug' => $getFirstWisePost['slug']]) }}"><div class="h-56 bg-cover text-center overflow-hidden" style="background-image: url('{{ $getFirstWisePost['featured_media_src_url']}}');" title=""></div></Link>
                        <div class="mt-3 bg-white rounded-b lg:rounded-b-none lg:rounded-r flex flex-col justify-between leading-normal">
                            <div class="">
                                ...
                                <Link href="{{ route('blog.single', ['slug' => $getFirstWisePost['slug']]) }}" class="text-gray-900 text-base font-bold text-lg mb-2 hover:text-indigo-600 transition duration-500 ease-in-out">{{ $getFirstWisePost['title']['rendered'] }}</Link>
                                <p class="text-gray-700 text-xs mt-2">{{ Str::limit(strip_tags(html_entity_decode($getFirstWisePost['excerpt']['rendered'])), 160) }}</p>
                            </div>
            ...
            c0-11.797,9.536-21.333,21.333-21.333s21.333,9.536,21.333,21.333V256z"
                              ...
                              <span class="flex bg-blue-600 text-xs text-white font-bold pr-2 pl-2 rounded">{{ \Carbon\Carbon::parse($getFirstWisePost['date'])->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="lg:border-l lg:pl-4">
                        @forelse($paginatedWise as $data)
